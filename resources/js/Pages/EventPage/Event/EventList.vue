...
<template>
  <div>
    <AuthenticatedLayout>
        ...
        <div class="w-full bg-white rounded-lg shadow-md dark:text-white dark:bg-gray-800">
          <div class="border-b border-gray-200 dark:border-none px-4 pb-5 mb-10">
            ...
            <div class="flex justify-end">
              <div class="relative">
                <TextInput ref="input" v-model.lazy="query" @keydown.enter="searchEvent" :placeholder="l.events.searchPlaceholder"
                    ...
                    <table class="min-w-full table-auto divide-y divide-gray-200">
                      <thead>
                        <tr>
                          ...
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.eventName }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.eventType }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.description }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.location }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.leadInstructor }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.assistInstructor }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.category }}
                          </th>
                          <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.createdAt }}
                          </th>
                          <th scope="col" class="py-3 text-end text-xs font-medium text-gray-500 uppercase">
                            {{ l.events.table.actions }}
                          </th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-gray-200">
                        <tr v-for="(event, index) in eventsData" :key="event.id">
                          <td class="py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">
                            {{ index + 1 }}
                          </td>
                          <td class="py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">
                            {{ event.event_name }}
                          ...
                          <td class="py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-block">
                              <button v-if="event.event_type == 2" type="button"
                            ...
                            <div class="inline-block">
                              <button type="button"
                                ...
                                aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-scale-animation-modal"
                                data-hs-overlay="#register-attendees-modal" v-on:click="getRegisterAttendees(event.id)">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                  stroke-width="1.5" stroke="currentColor" class="size-4">
                                  <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="inline-block">
                              <Link :href="`/event/eventList/view/${event.id}`"
                            ...
                            <div class="hs-tooltip inline-block">
                              <Link :href="`/event/eventList/edit/${event.id}`"
                            ...
                            <div class="hs-tooltip inline-block">
                              <Link :href="`/event/eventList/delete/${event.id}`"
            ...
            <div class="flex justify-end w-full">
              <div class="mb-10 my-5">
                <Link :href="link.url" v-for="(link, index) in eventLink" :key="index"
      ...
      <div id="register-attendees-modal"
        ...
        <div
          ...
          <div
            ...
            <div class="flex justify-between items-center py-3 px-4 border-b dark:border-neutral-700">
              <h3 id="register-attendees-modal" class="font-bold text-gray-800 dark:text-white">
                {{ l.events.attendeesList.title }}
              </h3>
              <button type="button"
                class="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:hover:bg-neutral-600 dark:text-neutral-400 dark:focus:bg-neutral-600"
                aria-label="Close" data-hs-overlay="#register-attendees-modal">
                  ...
                  <div class="p-1.5 min-w-full h-[400px] inline-block align-middle">
                    <div class="mb-3 flex items-center">
                      <div class="flex items-center">
                        ...
                        <div class="text-sm ms-2">{{ l.events.attendeesList.register }}</div>
                      </div>
                      <div class="flex items-center ms-3">
                        <div class="w-3 h-3 bg-red-100"></div>
                        <div class="text-sm ms-2">{{ l.events.attendeesList.unregister }}</div>
                      </div>
                    </div>
                    <div class="overflow-hidden">
                      <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                          <tr>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.no }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.attendeeName }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.attendeeEmail }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.attendeeDepartment }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.attendeePosition }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.registeredAt }}
                            </th>
                            <th scope="col" class="py-3 text-start text-xs font-medium text-gray-500 uppercase">
                              {{ l.events.attendeesList.table.createdAt }}
                            </th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                          <tr v-for="(attendee, index) in registerEvents" :key="attendee.id" :class="{
      ...
      <div id="register-attendees-qrcode-modal"
        class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none"
        role="dialog" tabindex="-1" aria-labelledby="hs-scale-animation-modal-label">
        <div
          ...
          <div
            ...
            <div class="flex justify-between items-center py-3 px-4 border-b dark:border-neutral-700">
              <h3 id="register-attendees-qrcode-modal" class="font-bold text-gray-800 dark:text-white">
                {{ event?.event_name }}
              </h3>
              <button type="button"
                class="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:hover:bg-neutral-600 dark:text-neutral-400 dark:focus:bg-neutral-600"
                aria-label="Close" data-hs-overlay="#register-attendees-qrcode-modal">
                <span class="sr-only">{{ l.events.button.close }}</span>
                <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                  stroke-linejoin="round">
                  <path d="M18 6 6 18"></path>
            ...
            <div class="p-4 overflow-y-auto">
              <div class="w-full flex justify-end">
                <a target="_blank" :href="`/event/QRCode/download/${event?.id}`"
                  class="inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                  <span>{{ l.events.button.download }}</span>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-4 inline ms-1">
                    <path stroke-linecap="round" stroke-linejoin="round"
