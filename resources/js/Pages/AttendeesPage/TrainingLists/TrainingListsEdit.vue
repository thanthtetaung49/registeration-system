...
console.log(props.trainingLists);

const form = useForm({
  training_name: props.trainingLists.training_name,
});

const page = usePage();
const l = page.props.language;

const updateTrainingLists = () =>
  form.post(`/attendees/trainingLists/update/${props.trainingLists.id}`);
</script>
      ...
      <div class="px-10 py-10">
        <header class="mb-10">
          <h3 class="text-gray-800 text-2xl pb-1 bold dark:text-white">{{
            l.attendees.trainingLists.trainingListsEditTitle }}</h3>
        ...
        <div class="w-full bg-white rounded-lg shadow-md dark:text-white dark:bg-gray-800">
          <div class="border-b border-gray-200 dark:border-none px-4 py-5">
            <div class="mt-5">
              <form v-on:submit.prevent="updateTrainingLists">
                <div>
                  <InputLabel :value="l.attendees.trainingLists.labels.trainingName"></InputLabel>
                  <TextInput :placeholder="l.attendees.trainingLists.placeholder.trainingName"
                    v-model="form.training_name" class="mt-3 w-[30%] text-sm"></TextInput>
